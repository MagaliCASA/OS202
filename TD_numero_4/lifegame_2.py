...
if __name__ == '__main__':
    ...
    if myRank == 0:
        ...
        comm.send(dico_patterns[choice][0],dest=1)
        comm.send(dico_patterns[choice][0],dest=2)
        comm.send(grid.cells[:,-1:absisses_loc+1],dest=1)
        comm.send(grid.cells[:,absisses_loc-1:1],dest=2)
        
    else:
        ordonnees,absisses = comm.recv(source=0)
        absisses_loc = absisses//2
        dim = (ordonnees,absisses_loc+2)
    ...
    while True:
        ...
        if myRank == 1 : 
            ...
            t2 = time.time()
            print(f"Temps calcul prochaine generation bloc gauche : {t2-t1:2.2e} secondes\n", end='')
            comm.send(local_grid.cells[:,1:-1],dest=0)
        ...
        if myRank == 2 : 
            ...
            diff = local_grid.compute_next_iteration()
            t2 = time.time()
            print(f"Temps calcul prochaine generation bloc droite : {t2-t1:2.2e} secondes\n", end='')
            comm.send(local_grid.cells[:,1:-1],dest=0)
        ...
        if myRank == 0 :
            ...
            grid.cells[:,:absisses_loc] = comm.recv(source=1)
            grid.cells[:,absisses_loc:] = comm.recv(source=2)
