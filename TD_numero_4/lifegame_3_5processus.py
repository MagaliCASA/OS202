...
class Grille:
    ...
    Exemple :
       grid = Grille( (10,10), init_pattern=[(2,2),(0,2),(4,2),(2,0),(2,4)], color_life=pg.Color("green"), color_dead=pg.Color("black"))
    """
    def __init__(self, dim, init_pattern=None, color_life=pg.Color("green"), color_dead=pg.Color("black")):
        import random
        self.dimensions = dim
...
if __name__ == '__main__':
    ...
    if myRank == 0:
        ...
        ordonnees,absisses = dico_patterns[choice][0]
        ordonnees_loc = ordonnees//2
        absisses_loc = absisses//2
        dim = (ordonnees_loc+2,absisses_loc+2)
        comm.send((ordonnees_loc,absisses_loc),dest=1)
        comm.send((ordonnees_loc,absisses_loc),dest=2)
        comm.send((ordonnees_loc,absisses_loc),dest=3)
        comm.send((ordonnees_loc,absisses_loc),dest=4)
        comm.send(grid.cells[:ordonnees_loc+2,:absisses_loc+2],dest=1)
        comm.send(grid.cells[:ordonnees_loc+2,absisses_loc-2:],dest=2)
        comm.send(grid.cells[ordonnees_loc-2:,:absisses_loc+2],dest=3)
        comm.send(grid.cells[ordonnees_loc-2:,absisses_loc-2:],dest=4)
        
    else:
        ordonnees_loc,absisses_loc = comm.recv(source=0)
        dim = (ordonnees_loc+2,absisses_loc+2)
        local_grid=Grille(dim)
        local_grid.cells = np.zeros(dim, dtype=np.uint8)

    if myRank==1:
        local_grid.cells = comm.recv(source=0)
    ...
    while True:
        ...
        t1 = time.time()
        if myRank == 1 : 
            comm.send((local_grid.cells[:,1],local_grid.cells[:,-2]),dest=2)
            ghost1_right,ghost1_left = comm.recv(source=2) 
            comm.send((local_grid.cells[1,:],local_grid.cells[-2,:]),dest=3)
            ghost1_low,ghost1_high = comm.recv(source=3) 
            local_grid.cells[:,0] = ghost1_left
            local_grid.cells[:,-1] = ghost1_right
            local_grid.cells[0,:] = ghost1_high
            local_grid.cells[-1,:] = ghost1_low
            diff = local_grid.compute_next_iteration()
            t2 = time.time()
            print(f"Temps calcul prochaine generation bloc en haut à gauche : {t2-t1:2.2e} secondes\n", end='')
            comm.send(local_grid.cells[1:-1,1:-1],dest=0)

        if myRank == 2 : 
            ghost2_right,ghost2_left = comm.recv(source=1)
            comm.send((local_grid.cells[:,1],local_grid.cells[:,-2]),dest=1)
            comm.send((local_grid.cells[1,:],local_grid.cells[-2,:]),dest=4)
            ghost2_low,ghost2_high = comm.recv(source=4) 
            local_grid.cells[:,0] = ghost2_left
            local_grid.cells[:,-1] = ghost2_right
            local_grid.cells[0,:] = ghost2_high
            local_grid.cells[-1,:] = ghost2_low
            diff = local_grid.compute_next_iteration()
            t2 = time.time()
            print(f"Temps calcul prochaine generation bloc en haut à droite : {t2-t1:2.2e} secondes\n", end='')
            comm.send(local_grid.cells[1:-1,1:-1],dest=0)

        if myRank == 3 : 
            comm.send((local_grid.cells[:,1],local_grid.cells[:,-2]),dest=4)
            ghost3_right,ghost3_left = comm.recv(source=4) 
            ghost3_low,ghost3_high = comm.recv(source=1)
            comm.send((local_grid.cells[1,:],local_grid.cells[-2,:]),dest=1) 
            local_grid.cells[:,0] = ghost3_left
            local_grid.cells[:,-1] = ghost3_right
            local_grid.cells[0,:] = ghost3_high
            local_grid.cells[-1,:] = ghost3_low
            diff = local_grid.compute_next_iteration()
            t2 = time.time()
            print(f"Temps calcul prochaine generation bloc en bas à gauche : {t2-t1:2.2e} secondes\n", end='')
            comm.send(local_grid.cells[1:-1,1:-1],dest=0)

        if myRank == 4 : 
            ghost4_right,ghost4_left = comm.recv(source=3)
            comm.send((local_grid.cells[:,1],local_grid.cells[:,-2]),dest=3)
            ghost4_low,ghost4_high = comm.recv(source=2)
            comm.send((local_grid.cells[1,:],local_grid.cells[-2,:]),dest=2) 
            local_grid.cells[:,0] = ghost4_left
            local_grid.cells[:,-1] = ghost4_right
            local_grid.cells[0,:] = ghost4_high
            local_grid.cells[-1,:] = ghost4_low
            diff = local_grid.compute_next_iteration()
            t2 = time.time()
            print(f"Temps calcul prochaine generation bloc en bas à droite : {t2-t1:2.2e} secondes\n", end='')
            comm.send(local_grid.cells[1:-1,1:-1],dest=0)
        
        if myRank == 0:
            appli.draw()
            t3 = time.time()
            print(f"Temps affichage : {t3-t1:2.2e} secondes\r", end='')
            grid.cells[:ordonnees_loc,:absisses_loc] = comm.recv(source=1)
            grid.cells[:ordonnees_loc,absisses_loc:] = comm.recv(source=2)
            grid.cells[ordonnees_loc:,:absisses_loc] = comm.recv(source=3)
            grid.cells[ordonnees_loc:,absisses_loc:] = comm.recv(source=4)
