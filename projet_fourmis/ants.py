...
if __name__ == "__main__":
    ...
    if rank==0:
        ...
        global_colony = Colony(nb_ants, pos_nest, max_life)
        # print(f"le processus {rank} s'occupe de {global_colony.directions.shape[0]} fourmis\n")


        # Envoi des seeds et des max_life pour chaque processus.
    ...
    if rank != 0:
        ...
        data_global_colony = comm.recv(source=0, tag=7)
    ...
    # iteration = 0
    # n = 10000 #Nb itérations maximal
    # duree_deb = time.time()

    snapshop_taken = False
    while True:# while iteration < n:
        if rank==0:
            ...
            for i in range(1,size):
                comm.send(pherom.pheromon,dest=i,tag=5)
        
        if rank != 0:
            # On reçoit les phéromones du Proc0
            pherom.pheromon = comm.recv(source=0,tag=5)

            # On itère la colonie et les phéromones
            food_counter = local_colony.advance(a_maze, pos_food, pos_nest, pherom, food_counter)
            pherom.do_evaporation(pos_food)

            # On met en forme les données relatives aux colonies pour les envoyer au proc0.
            colony_data = {'seeds': local_colony.seeds, 'is_loaded': local_colony.is_loaded, 'max_life': local_colony.max_life, 'age': local_colony.age, 'historic_path': local_colony.historic_path, 'directions': local_colony.directions,}
            
            # On groupe les données pour ne faire qu'un envoi.
            comm.send([(colony_data, start_ant, end_ant),pherom.pheromon,food_counter], dest=0, tag=2)

        if rank == 0:
            deb = time.time()

            # On oublie pas de remettre le food_counter à 0 car les autres processus gardent déjà le nombre courant de nourriture au nid.
            food_counter = 0

            for i in range(1, size):

                # On recoit les données groupés des différents processus.
                datas = comm.recv(source=i, tag=2)
                colony_data, start_ant, end_ant = datas[0]

                # Mise à jour des données globales après avoir récupéré les données de chaque processus.
                global_colony.max_life[start_ant:end_ant] = colony_data['max_life']
                global_colony.seeds[start_ant:end_ant] = colony_data['seeds']
                global_colony.is_loaded[start_ant:end_ant] = colony_data['is_loaded']
                global_colony.age[start_ant:end_ant] = colony_data['age']
                global_colony.historic_path[start_ant:end_ant] = colony_data['historic_path']
                global_colony.directions[start_ant:end_ant] = colony_data['directions']

                # On compare les phéromones reçus pour garder le maximum.
                # On met donc de côté les phéromones du processus 1, pour les comparer au processus d'après (s'il y en a un)
                if i==1:
                    # pherom_data_loc = comm.recv(source=i, tag=4)
                    pherom_data_loc = datas[1]
                else: 
                    # pherom_data = comm.recv(source=i, tag=4)
                    pherom_data = datas[1]
                    pherom_data_loc=np.maximum(pherom_data_loc, pherom_data)

                # On ajoute le food_counter de chaque processus
                food_counter += datas[2]
            
            # On met à jour les phéromones après avoir calculé le maximum 
            pherom.pheromon = pherom_data_loc
    ...
    # duree_end = time.time()
    # duree = duree_end - duree_deb
    # if rank == 0 :
    #     print(f"\nDurée de {n} itérations : {duree} secondes\n")
    # comm.Abort(0)
